...
use = egg:dummy_app""")
        ...
        with patch("wsgiref.simple_server.make_server", return_value=server_instance) as mock_make_server, \
...
def test_help_action(capsys):
    ...
    cm.add_command("dummy", MagicMock(return_value=MagicMock(deprecated=False, get_description=lambda: "Dummy command description")))
...
def test_setup_app(tmp_path):
    ...
    config_file.write_text(f"[app:main]\nuse=egg:fakeegg\n")

    fake_setup_app = MagicMock(return_value=0)

    with patch.object(sys, "argv", ["gearbox", "setup-app", "-c", str(config_file)]), \
         patch("gearbox.commands.setup_app.appconfig", return_value=MagicMock(
               context=MagicMock(entry_point_name="main", protocol="app", 
                                 distribution=MagicMock(
                                   read_text=MagicMock(return_value="fakemodule")
                                 )))), \
         patch.object(SetupAppCommand, "_import_module", return_value=MagicMock(
             setup_app=fake_setup_app
         )):
            main()
...
def test_scaffold(tmp_path):
    ...
    with patch.object(sys, "argv", ["gearbox", "scaffold", "model", "TestModel", "-l", str(lookup_dir), "-p", str(project_dir)]):
        main()
...
def test_patch(tmp_path):
    ...
    with patch("gearbox.commands.patch.PatchCommand._walk_flat", return_value=[str(test_file)]), \
         patch.object(sys, "argv", ["gearbox", "patch", str(test_file), "World", "-r", "Gearbox"]):
        main()
